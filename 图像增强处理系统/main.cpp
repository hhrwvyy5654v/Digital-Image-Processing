...
wchar_t* wstr;//保存导入图像，减少重复计算
void On_btnLoad_Click()
{
	
	
	if (ty == 0||ty==1||img.empty())//将载入图片特别判定
	{
	...
	else
	{
		...
		else
		{
			...
			case 2:
			{
					...
					gui.loadImg(wstr1, 315, 295, 220, 250);//先保存后导出
			...
			case 3:
			{
				...
				demo.Histogram_equalization(img);
				
				break;
			}
			case 4:
			{
				demo.Grayscale_stretching(img);
				break;
			}
			case 5:
			{
				demo.Mean_filtering(img);
				break;
			}
			case 6:
			{
				demo.Median_filtering(img);
				break;
			}
			case 7:
			{
				demo.Bilateral_filtering(img);
				break;
			}
			case 8:
			{
				demo.Sobel_operator(img);
				break;
			}
			case 9:
			{
				demo.Laplacian_operator(img);
				break;
			}
			case 10:
			{
				demo.Roberts_operator(img);
				break;
			}
			case 11:
			{
			...
			case 12:
			{
				demo.Otsu_segmentation(img);
				break;
			}
			case 13:
			{
				demo.Regional_growth_segmentation(img);
				break;
			}
			case 14:
			{
				demo.Watershed_segmentation(img);
				break;
			}
			}
		}
...
int num = 3;//btn计数从3开始
const wchar_t* btnName[20] = { L"sq", L"s",L"s", L"直方图均衡化", L"灰度拉伸", L"均值滤波", L"中值滤波", L"双边滤波", L"Sobel算子",L"Laplacian算子",L"Roberts算子",L"RGB转到HIS空间",L"大津法图像分割",L"区域生长分割",L"分水岭算法"};
void createBtn(int x, int y, int len)
{
	...
	for (int i = 0; i <= 5; i++)
	{
		...
		btnGon[num].Create(x, y, x + 200, y + 30, btnName[num], On_btnLoad_Click, num);	// ������ť�ؼ�
		num++;
		y += len;
	}
...
void initPage()
{
	...
	gui.puttext(680, 250, L"功能按钮", WHITE, 50, 0); // ���������ı���ؼ�
	gui.puttext(680, 280, L" ", WHITE, 30, 0); // ���������ı���ؼ�
}
...
int main()
{
	...
	while (true)
	{
		...
		if (msg.message == WM_LBUTTONDOWN)
		{
			...
			for (int i = 3; i < 15; i++)
			{
				if(btnGon[i].Check(msg.x, msg.y)) btnGon[i].OnMessage();
			}
		}
	}
